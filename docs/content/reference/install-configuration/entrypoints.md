...
| `forwardedHeaders.trustedIPs` | Set the IP from where Traefik Hub trusts the forwarded headers information (`X-Forwarded-*`). | | No |
...
| `http.tls` | Enable TLS on every router attached to the `entryPoint`. <br /> If no certificate are set, a default self-signed certificate is generates by Traefik Hub. <br /> We recommend to not use self signed certificates in production. | - | No |
| `http.tls.options` | Apply TLS options on every router attached to the `entryPoint`. <br /> The TLS options can be overidden per router. <br /> More information in the [dedicated section](../routing/kubernetes/http/tls/tlsoption.mdx) | - | No |
| `http.tls.certResolver` | Apply a certificate resolver on every router attached to the `entryPoint`. <br /> The TLS options can be overidden per router. <br /> More information in the [dedicated section](./tls/cert-resolvers.md) | - | No |
...
| `transport.`<br />`lifeCycle.`<br />`graceTimeOut` | Set the duration to give active requests a chance to finish before Traefik Hub stops. <br />Can be provided in a format supported by [time.ParseDuration](https://golang.org/pkg/time/#ParseDuration) or as raw values (digits).<br />If no units are provided, the value is parsed assuming seconds <br /> In this time frame no new requests are accepted.| 10s (seconds) | No |
| `transport.`<br />`lifeCycle.`<br />`requestAcceptGraceTimeout` | Set the duration to keep accepting requests prior to initiating the graceful termination period (as defined by the `transportlifeCycle.graceTimeOut` option). <br /> This option is meant to give downstream load-balancers sufficient time to take Traefik Hub out of rotation. <br />Can be provided in a format supported by [time.ParseDuration](https://golang.org/pkg/time/#ParseDuration) or as raw values (digits).<br />If no units are provided, the value is parsed assuming seconds| 0s (seconds) | No |
| `transport.`<br />`keepAliveMaxRequests` | Set the maximum number of requests Traefik Hub can handle before sending a `Connection: Close` header to the client (for HTTP2, Traefik Hub sends a GOAWAY). <br /> Zero means no limit. | 0 | No |
| `transport.`<br />`keepAliveMaxTime` | Set the maximum duration Traefik Hub can handle requests before sending a `Connection: Close` header to the client (for HTTP2, Traefik Hub sends a GOAWAY). Zero means no limit. | 0s (seconds) | No |
...
- You can attach a list of [middlewares](../routing/http/middlewares/overview.md)
to each EntryPoint.
...
(the Middlewares declared on the [IngressRoute](../routing/kubernetes/http/routers/httpingressroute.md#routesmiddlewares)
or the [Ingress](../routing/kubernetes/http/routers/ingress-annotations.md#middlewares)
...
As HTTP/3 actually uses UDP, when traefik hub is configured with a TCP `entryPoint`
...
When queuing Traefik Hub behind another load-balancer, make sure to configure 
...
to do
canary deployments against Traefik Hub itself. Like upgrading Traefik Hub version
